...
**Current blocker (verified code_implementer-1781084928224, 2026-06-10):** `/warm-index` mounted (~808 GiB avail; `warm_index_disk.avail_bytes=867428540416`). `S2_API_KEY_FILE=/run/secrets/s2-api-key` is configured on the pod but the mount is an **empty directory** (`configured_file_empty=true`, `empty_dir_mounts=1`, `probed_paths=33`) — apply `deploy/k8s/s2-api-key-secret.yaml` per [#6](https://github.com/li-langverse/li-research-ingest/issues/6). No `S2_API_KEY` in `li/.env.github`, `LI_SECRETS_DIR` (`/srv/homelab/li-research/secrets` empty), repo `.secrets/`, or homelab paths. `/warm-index/.secrets/` writable (mode 0700) for operator drop-in — `discover-s2-key.sh` now prints this path when writable. Staging/s2 remains sample-only (31,680 B, 0% of 1 GiB gate). arXiv OAI complete (~8.5 GiB, 2,363 files, 4 sets). Operator: add `S2_API_KEY` to `li/.env.github` (auto-probed), K8s secret (`./scripts/operator-wire-s2-key.sh`), homelab drop-in (`S2_API_KEY=... ./scripts/install-homelab-s2-secret.sh`), warm-index drop-in (`S2_API_KEY=... ./scripts/install-homelab-s2-secret.sh --dir /warm-index/.secrets`), or `.secrets/s2-api-key` (see [`.secrets/README.md`](.secrets/README.md)). Re-run `./scripts/unblock-r1b.sh` or `./scripts/agent-r1b-pass.sh --wait-for-key 3600` after wiring. Phase checklist: branch/runner/state/manifest/runbook **done**; s2_abstracts **blocked** pending key. Traceability: `/warm-index/staging/.ingest-run-state.json` and `.agent-r1b-report.json`.
